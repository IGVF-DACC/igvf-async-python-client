...
class InVitroSystem(BaseModel):
    ...
    """ # noqa: E501
    is_on_anvil: Optional[StrictBool] = Field(default=None, description="Indicates whether the data object has been submitted to AnVIL.")
    ...
    biosample_qualifiers: Optional[List[StrictStr]] = Field(default=None, description="An array of various cell states. This property provides additional information about a cell at a finer-grained level compared to what ontologies currently capture. For example, exhausted T-cells.")
    ...
    annotated_from: Optional[StrictStr] = Field(default=None, description="Links to the sample that was used as a reference for annotating this sample.")
    ...
    phenotypic_features: Optional[List[StrictStr]] = Field(default=None, description="A list of associated phenotypic features of the sample.")
    ...
    part_of: Optional[StrictStr] = Field(default=None, description="Links to a sample which represents a larger sample from which this sample was taken regardless of whether it is a tissue taken from an organism or smaller slices of a piece of tissue or aliquots of a cell growth.")
    ...
    virtual: Optional[StrictBool] = Field(default=None, description="Virtual samples do not represent actual physical entities from experiments, but instead capture metadata about hypothetical or inferred samples relevant to reported analysis results, including those derived through demultiplexing.")
    ...
    construct_delivery_methods: Optional[List[StrictStr]] = Field(default=None, description="Methods used to deliver construct libraries into the sample.")
    ...
    supersedes: Optional[List[StrictStr]] = Field(default=None, description="The sample(s) that this sample supersedes by virtue of being newer, better, or a fixed version of etc. than the one(s) it supersedes.")
    selection_conditions: Optional[List[StrictStr]] = Field(default=None, description="The conditions used for selecting the sample.")
    ...
    time_post_culture: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The time that elapsed since the biosample was placed into culture.")
    time_post_culture_units: Optional[StrictStr] = Field(default=None, description="The units of time that elapsed since the biosample was placed into culture.")
    ...
    origin_of: Optional[List[StrictStr]] = Field(default=None, description="The samples which originate from this sample, such as through a process of cell fate change or the introduction of a genetic material.")
    ...
    superseded_by: Optional[List[StrictStr]] = Field(default=None, description="Sample(s) this sample is superseded by virtue of those sample(s) being newer, better, or a fixed version of etc. than this one.")
    ...
    demultiplexed_to: Optional[List[StrictStr]] = Field(default=None, description="The in vitro system samples this in vitro system sample has been demultiplexed into.")
    __properties: ClassVar[List[str]] = ["is_on_anvil", "preview_timestamp", "release_timestamp", "publications", "taxa", "url", "sources", "lot_id", "product_id", "documents", "biosample_qualifiers", "lab", "award", "accession", "alternate_accessions", "collections", "status", "revoke_detail", "schema_version", "uuid", "notes", "aliases", "creation_timestamp", "submitted_by", "submitter_comment", "description", "annotated_from", "lower_bound_age", "upper_bound_age", "age_units", "sample_terms", "pooled_from", "originated_from", "treatments", "donors", "biomarkers", "embryonic", "modifications", "cellular_sub_pool", "phenotypic_features", "starting_amount", "starting_amount_units", "dbxrefs", "date_obtained", "part_of", "sorted_from", "sorted_from_detail", "virtual", "construct_library_sets", "moi", "construct_delivery_methods", "time_post_library_delivery", "time_post_library_delivery_units", "protocols", "supersedes", "selection_conditions", "classifications", "time_post_change", "time_post_change_units", "cell_fate_change_protocol", "demultiplexed_from", "passage_number", "targeted_sample_term", "growth_medium", "time_post_culture", "time_post_culture_units", "@id", "@type", "summary", "file_sets", "multiplexed_in", "parts", "sorted_fractions", "origin_of", "institutional_certificates", "superseded_by", "sex", "age", "upper_bound_age_in_hours", "lower_bound_age_in_hours", "pooled_in", "demultiplexed_to"]
    ...
    @field_validator('biosample_qualifiers')
    def biosample_qualifiers_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['6 days calcified', '10 days calcified', 'partially exhausted', 'exhausted']):
                raise ValueError("each list item must be one of ('6 days calcified', '10 days calcified', 'partially exhausted', 'exhausted')")
        return value
    ...
    @field_validator('construct_delivery_methods')
    def construct_delivery_methods_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['adenoviral transduction', 'adeno-associated viral (AAV) transduction', 'lipofectamine', 'electroporation', 'lentiviral transduction', 'transfection', 'transformation', 'nucleofection', 'transposase']):
                raise ValueError("each list item must be one of ('adenoviral transduction', 'adeno-associated viral (AAV) transduction', 'lipofectamine', 'electroporation', 'lentiviral transduction', 'transfection', 'transformation', 'nucleofection', 'transposase')")
        return value
    ...
    @field_validator('selection_conditions')
    def selection_conditions_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['35 degrees Celsius', 'glycine depletion', 'transfection of target gene', 'top 10% GFP-expressing cells', '1 mM 3-AT', '10 mM 3-AT', '25 mM 3-AT']):
                raise ValueError("each list item must be one of ('35 degrees Celsius', 'glycine depletion', 'transfection of target gene', 'top 10% GFP-expressing cells', '1 mM 3-AT', '10 mM 3-AT', '25 mM 3-AT')")
        return value
    ...
    def classifications_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['organoid', 'gastruloid', 'embryoid', 'cell culture', 'cell line', 'differentiated cell specimen', 'reprogrammed cell specimen', 'pooled cell specimen']):
                raise ValueError("each list item must be one of ('organoid', 'gastruloid', 'embryoid', 'cell culture', 'cell line', 'differentiated cell specimen', 'reprogrammed cell specimen', 'pooled cell specimen')")
        return value
    ...
    def growth_medium_validate_enum(cls, value):
        ...
        if value not in set(['DMEM with serum', 'DMEM without serum', 'SMBM with serum', 'SMBM without serum', '1 kPa hydrogel', '50 kPa hydrogel', 'tissue culture plastic plates (no hydrogel)']):
            raise ValueError("must be one of enum values ('DMEM with serum', 'DMEM without serum', 'SMBM with serum', 'SMBM without serum', '1 kPa hydrogel', '50 kPa hydrogel', 'tissue culture plastic plates (no hydrogel)')")
        return value

    @field_validator('time_post_culture_units')
    def time_post_culture_units_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "is_on_anvil": obj.get("is_on_anvil"),
            ...
            "biosample_qualifiers": obj.get("biosample_qualifiers"),
            "lab": obj.get("lab"),
            "award": obj.get("award"),
            ...
            "annotated_from": obj.get("annotated_from"),
            ...
            "moi": obj.get("moi"),
            "construct_delivery_methods": obj.get("construct_delivery_methods"),
            ...
            "selection_conditions": obj.get("selection_conditions"),
            ...
            "time_post_culture": obj.get("time_post_culture"),
            "time_post_culture_units": obj.get("time_post_culture_units"),
            "@id": obj.get("@id"),
            "@type": obj.get("@type"),
