...
class AlignmentFile(BaseModel):
    ...
    read_count: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Number of reads in a bam file. Including both mapped, unmapped, and multi-mapped read counts.")
    ...
    assembly: Optional[StrictStr] = Field(default=None, description="The assembly associated with the alignment file.")
    __properties: ClassVar[List[str]] = ["anvil_url", "base_modifications", "preview_timestamp", "controlled_access", "release_timestamp", "reference_files", "filtered", "documents", "lab", "award", "accession", "alternate_accessions", "collections", "status", "revoke_detail", "schema_version", "uuid", "notes", "aliases", "creation_timestamp", "submitted_by", "submitter_comment", "description", "analysis_step_version", "content_md5sum", "content_type", "dbxrefs", "derived_from", "derived_manually", "file_format", "file_format_specifications", "file_set", "file_size", "md5sum", "submitted_file_name", "upload_status", "validation_error_detail", "checkfiles_version", "checkfiles_timestamp", "supersedes", "read_count", "redacted", "@id", "@type", "summary", "integrated_in", "input_file_for", "gene_list_for", "loci_list_for", "quality_metrics", "superseded_by", "assay_titles", "preferred_assay_titles", "preferred_assay_slims", "workflows", "href", "s3_uri", "upload_credentials", "content_summary", "transcriptome_annotation", "assembly"]
    ...
    def file_format_validate_enum(cls, value):
        ...
        if value not in set(['bam', 'cram']):
            raise ValueError("must be one of enum values ('bam', 'cram')")
        return value
