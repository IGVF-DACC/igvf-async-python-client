...
class IndexFile(BaseModel):
    ...
    reference_files: Optional[List[StrictStr]] = Field(default=None, description="The reference files of the file that this index file is derived from.")
    __properties: ClassVar[List[str]] = ["anvil_url", "catalog_collections", "catalog_class", "catalog_notes", "preview_timestamp", "release_timestamp", "controlled_access", "documents", "lab", "award", "accession", "alternate_accessions", "collections", "status", "revoke_detail", "schema_version", "uuid", "notes", "aliases", "creation_timestamp", "submitted_by", "submitter_comment", "description", "analysis_step_version", "content_md5sum", "content_type", "dbxrefs", "derived_from", "derived_manually", "file_format", "file_format_specifications", "file_set", "file_size", "md5sum", "submitted_file_name", "upload_status", "validation_error_detail", "checkfiles_version", "checkfiles_timestamp", "supersedes", "@id", "@type", "summary", "integrated_in", "input_file_for", "gene_list_for", "loci_list_for", "quality_metrics", "superseded_by", "assay_titles", "preferred_assay_titles", "preferred_assay_slims", "workflows", "href", "s3_uri", "upload_credentials", "assembly", "transcriptome_annotation", "filtered", "redacted", "reference_files"]
    ...
    def file_format_validate_enum(cls, value):
        ...
        if value not in set(['bai', 'crai', 'tbi']):
            raise ValueError("must be one of enum values ('bai', 'crai', 'tbi')")
        return value
