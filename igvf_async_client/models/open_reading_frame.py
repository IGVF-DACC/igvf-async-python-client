...
class OpenReadingFrame(BaseModel):
    ...
    transcript_ids: Optional[List[Annotated[str, Field(strict=True)]]] = Field(default=None, description="ENSEMBL TranscriptID of official nomenclature approved transcript/isoform of that gene")
    barcode_sequence: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="DNA sequence of the unique barcode identifier of the overexpressed construct")
    orf_sequence: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="DNA sequence of the ORF specifically (this is a subset of the Insert DNA sequence)")
    insert_sequence: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="DNA sequence of longer construct")
    ...
    __properties: ClassVar[List[str]] = ["preview_timestamp", "release_timestamp", "status", "schema_version", "uuid", "notes", "aliases", "creation_timestamp", "submitted_by", "submitter_comment", "description", "lab", "award", "orf_id", "genes", "protein_id", "transcript_ids", "barcode_sequence", "orf_sequence", "insert_sequence", "dbxrefs", "pct_identical_protein", "pct_coverage_protein", "pct_coverage_orf", "@id", "@type", "summary"]
    ...
    def orf_id_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(CCSBORF|TFORF)[0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^(CCSBORF|TFORF)[0-9]+$/")
        return value
    ...
    @field_validator('barcode_sequence')
    def barcode_sequence_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^[ACGT]+$", value):
            raise ValueError(r"must validate the regular expression /^[ACGT]+$/")
        return value

    @field_validator('orf_sequence')
    def orf_sequence_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value
    ...
    @field_validator('insert_sequence')
    def insert_sequence_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "transcript_ids": obj.get("transcript_ids"),
            "barcode_sequence": obj.get("barcode_sequence"),
            "orf_sequence": obj.get("orf_sequence"),
            "insert_sequence": obj.get("insert_sequence"),
