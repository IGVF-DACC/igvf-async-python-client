...
class SearchResultItem(BaseModel):
    ...
    # data type: PseudobulkSet
    oneof_schema_26_validator: Optional[PseudobulkSet] = None
    # data type: Gene
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, PredictionSet):
            ...
            match += 1
        # validate data type: PseudobulkSet
        if not isinstance(v, PseudobulkSet):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PseudobulkSet`")
        else:
            match += 1
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # check if data type is `PseudobulkSet`
        if _data_type == "PseudobulkSet":
            instance.actual_instance = PseudobulkSet.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into PseudobulkSet
        try:
            instance.actual_instance = PseudobulkSet.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
